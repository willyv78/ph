...
$res_habita = registroCampo("rmb_residente_x_aptos rxa", "r.rmb_residente_id, r.rmb_residente_nom, r.rmb_residente_ape, r.rmb_vinculo_id", "LEFT JOIN rmb_residente r USING(rmb_residente_id) WHERE rxa.rmb_aptos_id = '$apto_id' AND rxa.rmb_tres_id = '2'", "", "");
?>
    ...
    <table class="table table-hover">
        ...
        if($res_habita){
            if(mysql_num_rows($res_habita) > 0){
                while ($row_habita = mysql_fetch_array($res_habita)) {?>
                    <tr class="">
                        <td class='col-xs-7 col-sm-4 col-md-2 col-lg-2 text-center'><?php echo $row_habita[1];?></td>
                        <td class='hidden-xs col-sm-4 col-md-3 col-lg-3 text-center'><?php echo $row_habita[2];?></td>
                        <td class='hidden-xs hidden-sm col-md-3 col-lg-3 text-center'><?php echo nombreCampo($row_habita[3], "rmb_vinculo");?></td>
                        <td class='col-xs-5 col-sm-4 col-md-4 col-lg-4 text-center'>
                            <div name="<?php echo $row_habita[0];?>" class="input-group input-group-md btn_est">
...
<script>accionHabitantes();</script>