...
$res_val = registroCampo("rmb_eva_cate c", "c.rmb_eva_cate_id, c.rmb_eva_cate_nom", "", "", "ORDER BY c.rmb_eva_cate_nom ASC");
?>
        ...
        if (mysql_num_rows($res_val) > 0) {
          while($row_val = mysql_fetch_array($res_val)){?>
            <tr>
              ...
              <td id="lista_categoria" class="vertical-middle" name="<?php echo $row_val[0];?>">
    ...
    function crearTabla (selector) {
      ...
      setTimeout(function(){
        $('#tabla tfoot th').each(function() {
          ...
          $(this).html('<input class="form-control" type="text" placeholder=" Busqueda por ' + title + ' de la categoría" />');
        });
