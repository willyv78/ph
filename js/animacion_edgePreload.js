...
if(!AdobeEdge.yepnope){
	(function(a,b,c){
		...
		B.errorTimeout=1e4,
		null==b.readyState&&b.addEventListener&&(b.readyState="loading",b.addEventListener("DOMContentLoaded",A=function(){b.removeEventListener("DOMContentLoaded",A,0),b.readyState="complete"},0)),
			a.yepnope=k(),
			...
			a.yepnope.injectCss=function(a,c,d,e,g,i){var e=b.createElement("link"),j,c=i?h:c||f;e.href=a,e.rel="stylesheet",e.type="text/css";for(j in d)e.setAttribute(j,d[j]);g||(n.parentNode.insertBefore(e,n),m(c,0))}
		})
	(this,document);
...
(function(compId){
	...
	AdobeEdge.requestResources=AdobeEdge.requestResources||function(a,d){
		...
		for(b=0;b<k;b++){
			c=a[b];
			if("string"===typeof c)
				0===c.indexOf("//")&&0===window.location.href.indexOf("file://")&&(c="http:"+c),url=c,c={load:url};
			else if(c.load&&0===c.load.indexOf("//")&&0===window.location.href.indexOf("file://")&&(c.load="http:"+c.load),url=c.yep||c.load,c.callback){
				var g=c.callback;
				c.callback=function(a,b,c){g(a,b,c)&&d(a,b,c)}
			}
	...
	function loadResources(a,d){AdobeEdge.preload=AdobeEdge.preload||[];AdobeEdge.preload.q=AdobeEdge.preload.q||[];d||!isCapable()?filesToLoad=a:AdobeEdge.preload.busy?AdobeEdge.preload.q.push({files:a,callback:edgeCallback}):AdobeEdge.requestResources(a,edgeCallback)}
	function splitUnits(a){var d={};d.num=parseFloat(a);"string"==typeof a&&(d.units=a.match(/[a-zA-Z%]+$/));d.units&&"object"==typeof d.units&&(d.units=d.units[0]);return d}
	function defaultUnits(a){var d=a;"auto"!==a&&((a=splitUnits(a))&&a.units||(d+="px"));return d}
	function findNWC(a,d){
		if(-1!=String(a.className).indexOf(d))
			return a;
	...
	function wrapForStageScaling(a){if(!isWrapped(a)){var d=document.createElement("div"),b=document.createElement("div");d.className="flow-wrapper";b.className="center-wrapper";d.style.width="1px";d.appendChild(b);parent(a).insertBefore(d,a);b.appendChild(a)}}
	...
	function simpleContent(a,d,b){
		...
		for(var n=0;n<a.length;n++){
			...
			if("text"==b.type){
				if(c=b.font)
					c[0]&&""!==c[0]&&(h.fontFamily=c[0]),
					"object"!=typeof c[1]&&(c[1]=[c[1]]),
					c[1][1]||(c[1][1]="px"),
					c[1][0]&&""!==c[1][0]&&(h.fontSize=c[1][0]+c[1][1]),
					c[2]&&""!==c[2]&&(h.color=safeColor(c[2])),
					c[3]&&""!==c[3]&&(h.fontWeight=c[3]),
					c[4]&&""!==c[4]&&(h.textDecoration=b.font[4]),
					c[5]&&""!==c[5]&&(h.fontStyle=b.font[5]);
			...
			f.appendChild(k);
			if(b.c)
				for(h=0;h<b.c.length;h++)
					simpleContent(b.c[h],d,k)
		}
	}
	var fnCycle=function(a){a?fnCycle&&setTimeout(fnCycle,20):a={event:"loading",progress:0};loadingEvt&&loadingEvt(a)},
		aBootcompsLoaded=[];
	window.AdobeEdge.bootstrapListeners||(window.AdobeEdge.bootstrapListeners=[]);
	window.AdobeEdge.bootstrapCallback=function(a){window.AdobeEdge.bootstrapListeners.push(a);if(0<aBootcompsLoaded.length)for(var d=0;d<aBootcompsLoaded.length;d++)a(aBootcompsLoaded[d])};
	window.AdobeEdge.preloadComplete||(window.AdobeEdge.preloadComplete={});
	window.AdobeEdge.preloadComplete[compId]=function(a){AdobeEdge.$_(".edgePreload"+a).css("display","none");fnCycle=null;loadingEvt&&loadingEvt({event:"done",progress:1,reason:"complete"});aBootcompsLoaded.push(a);for(var d=window.AdobeEdge.bootstrapListeners.length,b=0;b<d;b++)try{window.AdobeEdge.bootstrapListeners[b](a)}catch(k){console.log("bootstrap error "+k)}};
	...
	if(document.addEventListener){window.addEventListener("load",onDocLoaded,false);}
	else if(document.attachEvent){window.attachEvent("onload",onDocLoaded);}
	requiresSVG=false;
	doDelayLoad=true;
	htFallbacks={"edge_includes/jquery-2.0.3.min.js":"//ajax.googleapis.com/ajax/libs/jquery/2.0.3/jquery.min.js","edge_includes/edge.3.0.0.min.js":"//animate.adobe.com/runtime/3.0.0/edge.3.0.0.min.js"};
	aLoader=[{load:"//ajax.googleapis.com/ajax/libs/jquery/2.0.3/jquery.min.js",callback:function(result,key){if(!window.jQuery){yepnope({load:"edge_includes/jquery-2.0.3.min.js",callback:edgeCallback});return false;}return true;}},
		{load:"//animate.adobe.com/runtime/3.0.0/edge.3.0.0.min.js",callback:function(result,key){if(!window.AdobeEdge){yepnope({load:"edge_includes/edge.3.0.0.min.js",callback:edgeCallback});return false;}return true;}},
		{load:"js/animacion_edge.js"},
		{load:"js/animacion_edgeActions.js"}];
	...
	var plSTF="width",ctrPlS="none",minPlW="0",maxPlW=undefined,plWidth="1440px",plHeight="400px";
	preContent={dom:[]};
	//simpleContent
	dlContent={dom:[]};
	//simpleContent
